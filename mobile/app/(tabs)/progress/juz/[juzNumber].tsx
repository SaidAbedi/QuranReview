      ...
      {!loading && !error && detail && (
        <ScrollView style={styles.container} contentContainerStyle={styles.content}>
          ...
          <View style={styles.summaryCard}>
            <Text style={styles.summaryTitle}>Juz {juzNum}</Text>
            <View style={styles.summaryStats}>
              ...
              <View style={styles.summaryStat}>
                <Text style={[styles.summaryNum, { color: C.gold }]}>{detail.totalPagesInJuz}</Text>
                <Text style={styles.summaryLabel}>Total Pages</Text>
              </View>
            </View>
            <View style={styles.barTrack}>
              <View
                style={[styles.barFill, { width: `${pct}%` as `${number}%`, backgroundColor: pct === 100 ? C.green : C.gold }]}
              />
            </View>
          ...
          {detail.pages.length === 0 ? (
            <Text style={styles.empty}>No pages assigned in this juz yet.</Text>
          ) : (
...
const styles = StyleSheet.create({
  ...
  summaryTitle:  { fontSize: 22, fontWeight: '800', color: C.text, marginBottom: 12 },
